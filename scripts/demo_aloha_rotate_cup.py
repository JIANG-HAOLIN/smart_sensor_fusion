            ...
            for idx1, loader in enumerate([train_inference_loaders]):
                ...
                for t, batch in enumerate(loader):
                    if t % query_frequency == 0:
                        ...
                        action = torch.cat([action, batch["traj"]["gripper"]["action"].unsqueeze(-1).float()], dim=-1).to(args.device)
                        # print(qpos.shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', type=str,
                        default="../checkpoints/rotate_cup/name=alohaname=vae_resnet_qposaction=positionname=coswarmuplr=4e-05source=Trueresized_height_v=240resized_width_v=320batch_size=64_06-18-11:21:31")
